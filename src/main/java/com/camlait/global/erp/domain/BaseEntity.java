...
public abstract class BaseEntity implements Serializable {
	...
	public abstract BaseEntity init();
	...
	/**
	 * Scan the current entity in order to find all collection that need to be
	 * lazy initialized then initialized them.
	 * 
	 * @return The current entity after lazy initialized collections.
	 */
	public <T extends BaseEntity> T lazyInit() {
		Stream.of(this.getClass().getDeclaredFields()).filter(this::canBeLazyInit)
	...
	private Boolean canBeLazyInit(Field f) {
		final Boolean isAnnotated = Stream.of(f.getDeclaredAnnotations()).anyMatch(a -> {
			return EntityHelper.isTypeOf(a.annotationType(), ManyToMany.class)
					|| EntityHelper.isTypeOf(a.annotationType(), OneToMany.class);
		});
	 ...
	 * 
	 * @param f
	 *            Field that the value need to be retrieved.
