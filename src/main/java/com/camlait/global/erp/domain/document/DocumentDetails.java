...
public class DocumentDetails extends BaseEntity {
        ...
        if (document != null && document.isBusinessDocument()) {
            ...
            if (!CollectionUtils.isNullOrEmpty(taxes)) {
                final Set<DocumentDetailsTax> lt = taxes.stream().map(t -> {
                    return DocumentDetailsTax.builder().documentDetails(this).docDetailId(this.getDocDetailId()).taxRate(t.getPercentageValue()).tax(t)
                            .taxId(t.getTaxId()).build();
    ...
    public DocumentDetails init() {
        ...
        setDocumentDetailsTaxes(
                documentDetailsTaxes == null ? Lists.newArrayList() : documentDetailsTaxes.stream().map(ddt -> ddt.init()).collect(Collectors.toList()));
        return this;
    }
