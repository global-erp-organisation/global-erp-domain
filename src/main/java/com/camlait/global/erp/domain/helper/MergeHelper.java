 ...
 * <p>
 * {@code}1: mergeBean = new MergeHelper() or mergeBeanUtil = new
 * MergeHelper(builderCondition)
 * <p>
...
public class MergeHelper extends BeanUtilsBean {
	 ...
	 * <p>
	 * Here, only null fields or empty collections are updated in the
	 * destination object with corresponding non null fields value from the
	 * source.
	 */
	public MergeHelper() {
	 ...
	 */
	@Override
	public void copyProperty(Object destination, String fieldName, Object sourceValue)
	...
	private Boolean canBeOverrides(Object sourceValue, Object destinationValue) {
		if (sourceValue != null) {
			if (sourceValue instanceof Collection && destinationValue != null) {
				if (CollectionUtils.isNullOrEmpty((Collection) destinationValue)) {
					return !((Collection) sourceValue).isEmpty();
				}
			} else if (sourceValue instanceof Map && destinationValue != null) {
				if (destinationValue != null && ((Map) destinationValue).isEmpty()) {
					return !((Map) sourceValue).isEmpty();
				}
			} else {
				return destinationValue == null || overrideNumber(sourceValue, destinationValue);
			}
		}
		return false;
	}

	private Boolean overrideNumber(Object sourceValue, Object destinationValue) {
		if (sourceValue != null && sourceValue instanceof Number) {
			final Number value = (Number) destinationValue;
